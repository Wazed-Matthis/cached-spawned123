...
// Equivalent to specifying `type = "UnboundCache<(u32), u32>", create= "{
// UnboundCache::new() }"`
#[cached]
fn fib(n: u32) -> u32 {
  if n == 0 || n == 1 {
...
pub fn main() {
  ...
  let r = with_cached_flag_result("a".to_string())
    .expect("with_cached_flag_result failed");
  println!("was cached: {}", r.was_cached);
  ...
  let r = with_cached_flag_option("a".to_string())
    .expect("with_cached_flag_result failed");
  println!("was cached: {}", r.was_cached);
